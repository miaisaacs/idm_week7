...
# [LQ3] Modify the sample code (no intervention included) to run the Ebola stochastic model with 
# no intervention during the first 8 weeks and with intervention starting Week 9 (Day 63). 
# For this simulation, assume when intervention is implemented, transmission rate in the community 
# is reduced by 88% (, i.e., to 12% of the initial level), and that transmission in both the 
# hospitals and funeral ceremonies is completed eliminated. 
# Submit your code in your report. Also test run your code; plot and show the simulated weekly new 
# (1pt)
# Hint: think about how to control the simulation time and what parameters you would need to use 
# depending on whether intervention is in place. 
z=.88; # intervention in the community
z.H=1; # intervention in the hospital
z.F=1; # intervention in safe burial

while((I>0 | E>0 | H>0 | FF>0) & S>0){ # simulate epid for the first 9 week without ctrl
  # step 1: compute the transition rates
  if(tm<9){
    beta.I=.588; # transmission rate in community
    beta.H=.794; # transmission rate in hospital
    beta.F=7.653; # transmission rate at funerals
...
for(ir in 1:num_run){
  ...
  colnames(res)=c('time','S','E','I','H','FF','R','cumI')
  ## compile results to weekly interval
  num_wk=ceiling(res[nrow(res),'time']) # epidemic duration
  wkly.res=matrix(NA,num_wk,8); colnames(wkly.res)=c('time','S','E','I','H','FF','R','cumI')
  num_events=NULL;
  for(wk in 1:num_wk){
    idx=tail(which(res[,'time']<wk),1);
    # idx also records the number of events happened from time 0 to current week
...
# Assuming 'out' is your results matrix with columns: num_run, duration, peak, final.size

# Extracting the columns for easier reference
duration_data <- out[, 'duration']
peak_data <- out[, 'peak']
final_size_data <- out[, 'final.size']

# Setting up the plotting area to accommodate multiple plots
par(mfrow = c(1, 3))  # Adjust the layout as needed

# Histogram for 'duration'
hist(duration_data, 
     main = 'Distribution of Epidemic Duration',
     xlab = 'Duration (weeks)',
     col = 'lightblue',
     border = 'black',
...
# Histogram for 'peak'
hist(peak_data, 
     main = 'Distribution of Peak Incidence',
     xlab = 'Peak Incidence',
     col = 'lightgreen',
     border = 'black',
     breaks = 20)  # Adjust the number of bins as needed

# Histogram for 'final.size'
hist(final_size_data, 
...
# Resetting the plotting area to a single plot
par(mfrow = c(1, 1))
